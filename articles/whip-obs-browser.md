...
WHIP対応のサーバーを利用する場合、ブラウザではそのまま接続できない場合もあります。OBS対応をうたっているサーバーの場合は、ブラウザでOBSのフリをすることで接続できるケースもあります。（※OBSのフリをして接続することが利用規約に反する場合もあるので、ご注意ください）
...
サーバーによっては、他のWebサイト/WebアプリからのアクセスがCORSに引っかかる事があります。
詳細は省略しますが、中継サーバーを用意してPOST等のHTTPリクエストを中継してあげることで、CORSを回避できます。
